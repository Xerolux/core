...
from modules.backup_clouds.seafile.config import SeafileBackupCloud, SeafileBackupCloudConfiguration
...
def _parse_seafile_base_url_and_user(config: SeafileBackupCloudConfiguration, backup_filename: str):
    """
    Liefert Basis-URL und Benutzer.
    Zusätzlich wird der WebDAV-Pfad zum Backup-Verzeichnis zurückgegeben.
    """
    upload_url = config.ip_address.rstrip('/') if config.ip_address else ""
    user = config.user
    base_path = "/seafdav"
    if getattr(config, 'path', None):
        base_path += f"/{config.path.strip('/')}"
    return upload_url, user, base_path



def _list_backups(config: SeafileBackupCloudConfiguration,
    ...
    upload_url, user, base_path = _parse_seafile_base_url_and_user(config, backup_filename)
    ...
    if base_name.endswith(".openwb-backup.gpg"):
        ...
        log.warning("Seafile Retention: Unerwartetes Backup-Dateimuster: %s", base_name)
        return []
...
<d:propfind xmlns:d="DAV:">
  <d:prop>
    ...
    if not response.ok:
        log.warning("Seafile PROPFIND für Backup-Liste fehlgeschlagen: %s %s",
...
def _enforce_retention(config: SeafileBackupCloudConfiguration, backup_filename: str) -> None:
    """
    Löscht alte Seafile-Backups, so dass höchstens max_backups Dateien mit dem
    ...
    for name in to_delete:
        ...
        try:
            log.info("Lösche altes Seafile-Backup: %s", delete_path)
            ...
            if not resp.ok:
                log.warning("Löschen des Seafile-Backups '%s' fehlgeschlagen: %s %s",
                            delete_path, resp.status_code, resp.reason)
        except Exception as error:
            log.error("Fehler beim Löschen alter Seafile-Backups (%s): %s",
                      delete_path, str(error).split("\n")[0])


def upload_backup(config: SeafileBackupCloudConfiguration, backup_filename: str, backup_file: bytes) -> None:
    upload_url, user, base_path = _parse_seafile_base_url_and_user(config, backup_filename)
    ...
    try:
        ...
        log.error("Fehler bei der Bereinigung alter Seafile-Backups: %s",
                  str(error).split("\n")[0])


def create_backup_cloud(config: SeafileBackupCloud):
    def updater(backup_filename: str, backup_file: bytes):
        upload_backup(config.configuration, backup_filename, backup_file)
    return updater


device_descriptor = DeviceDescriptor(configuration_factory=SeafileBackupCloud)
